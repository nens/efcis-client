  ...
  render: function() {
    ...
    var filteredParameters = self.props.parameters.filter(function(obj) {
      if (
        obj.par_oms
          .toLowerCase()
          .indexOf(self.props.filterString.toLowerCase()) != -1
      ) {
        return obj;
      }
      if (
    ...
    if (filteredParameters.length === 0)
      return <p className="bg-warning">Geen resultaten...</p>;
    ...
    var parametersList = filteredParameters.map(function(d, i) {
      ...
      if (d.par_oms_nl) {
        itemLabel = itemLabel + " - " + d.par_oms_nl;
      }

      return (
        <li key={i} onClick={e => dispatch(addParameterToSelection(d))}>
    ...
    return (
      <div>
        <ul className="from" id="parameter-list">
          {parametersList}
        </ul>
      </div>
    );
...
var SelectedParameterList = React.createClass({
  render: function() {
    ...
    const parametersList = data.map((parameter, i) => {
      return (
        <li key={i} onClick={() => dispatch(removeParameterFromSelection(parameter.id))}>
          <a ref="parameter" id={parameter.id} style={{ cursor: "pointer" }}>
            {parameter.par_oms}{" "}
            {parameter.par_oms_nl ? parameter.par_oms_nl : null}
          </a>
        </li>
      );
    });
    return (
      <div>
        <ul id="parameter-list" className="to" ref="selected_parameters">
          {parametersList}
        </ul>
      </div>
    );
...
class SelectParameterList extends Component {
  constructor(props) {
    ...
    this.selectTreeNodesFromStorage = this.selectTreeNodesFromStorage.bind(
      this
    );
  }
  ...
  handleClearSelection() {
    ...
    swal(
      ...
      function() {
        ...
        swal(
          "Selectie verwijderd",
          "De selectie is succesvol verwijderd",
          "success"
        );
      }
    );
  ...
  handleSaveState() {
    ...
    var selectedTreeNodes = $("#parameter-group-tree")
      .jstree("get_selected")
      .toString();
    ...
    return (
      <div>
        <div className="row">
          ...
          <div className="col-md-4" style={{ height: 50 }}>
            <div>
              Geselecteerde Parameters ({this.props.opnames.parameterIds.length})
            </div>
          </div>
        </div>
        <div className="row">
          ...
          >
            <SelectedParameterList
              {...this.props}
              data={this.props.opnames.parameters}
            />
          </div>
        </div>
      </div>
